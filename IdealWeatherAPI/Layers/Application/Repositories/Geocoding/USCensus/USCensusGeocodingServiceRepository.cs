...
using IdealWeatherAPI.Layers.Application.Repositories.Geocoding.USCensus.Models;
using IdealWeatherAPI.Layers.Application.Repositories.Geocoding.USCensus.Models.Benchmarks;
using IdealWeatherAPI.Layers.Application.Repositories.Geocoding.USCensus.Utilities;
using IdealWeatherAPI.Layers.Application.Repositories.Geocoding.USCensus.Validations;
using IdealWeatherAPI.Layers.Domain.ExceptionManagement.Exceptions;
using IdealWeatherAPI.Layers.Domain.Exceptions;
using IdealWeatherAPI.Layers.Domain.Geocoding;

using Newtonsoft.Json;

namespace IdealWeatherAPI.Layers.Application.Repositories.Geocoding.USCensus
{
    public class USCensusGeocodingServiceRepository : IGeocodingRepository
    {
        ...
        public async Task<Coordinates?> GetCoordinates(Address address)
        {
            ...
            var apiResponse = await _geocodeClient.GetFromJsonAsync<USCensusGeocodeResponse>(uri);
        ...
        private async Task<string> GetLatestBenchmarkVersionAsync()
        {
            ...
            var apiResponse = await _geocodeClient.GetFromJsonAsync<BenchmarkResult>(uri);
